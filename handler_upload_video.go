...
func (cfg *apiConfig) handlerUploadVideo(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Unable to provide video", err)
		return
	}
...
func (cfg *apiConfig) dbVideoToSignedVideo(video database.Video) (database.Video, error) {
	if video.VideoURL == nil {
        return video, errors.New("videoURL is nil")
    }

	params := strings.Split(*video.VideoURL, ",")
	presignedURL, err := generatePresignedURL(cfg.s3Client, params[0], params[1], 15*time.Minute)
	if err != nil {
		return video, err
	}
